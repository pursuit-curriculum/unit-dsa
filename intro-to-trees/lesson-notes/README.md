...
In the image above, each node contains a name and is related to one or more other nodes through position _and hierarchy._ For example, the name "Rohan" comes after "John" but is also at a higher level than "Sal" and "Emma".
...
Trees are valuable in part because they allow for representing hierarchy. Both the file system and HTML use this hierarchy for different features. However, trees can also be advantageous when constructed in specific ways, as they can improve the search for particular data.
...
In the tree, you only need three operations. You will check to see if the root node stores the value, which it does not. Then you could compare the value you're looking for (i.e., `99`) to the value of the root node (i.e., `50`). Because `99` is greater than `50,` you _can be certain_ that the value is to the right of the root node. You would do the same check and comparison at the next node, which is `67`.
...
In the worst-case scenario, binary search trees are no better than linear search. Both have a worst-case time complexity of `O(n)`. What would a binary search tree look like to fit the worst-case scenario?
...
You can loosely represent a tree with the following two classes and code.
...
The code above is set up to create a general tree. What would need to change in the classes to help enforce a binary tree?
...
Update the code above to add more nodes to the tree. You may use the image below or add your names to the tree. Add at least enough nodes, so the tree has a height of 3 or more.
...
Look at the `getDepth()` method in the `Tree` class. The `console.log()` statements are unnecessary for the code to work, although they may help you understand what is occurring. The code works as follows:
...
Next, a new variable is set, `results`. This variable is set to the return value of using `.map()` on all children and calling `.getDepth()` on each child node. For each iteration, `1` is added to the return value of `getMethod()` to represent the new edge created.
...
To track this as it happens, run the code with the `console.log()` statements. You should see an output that looks like this:
...
For example, `Depths below Node B [ 1 ]` represents the results below the `B` node. `B` only has one child, `E`. Therefore it stores a value of `0` + `1`, resulting in an array of `[ 1 ]`.

The last line before the result, `Depths below Node A [ 2, 1, 3 ]`, shows the largest depths of each of Node `A`'s children. As you can see, it matches the structure of the tree.
